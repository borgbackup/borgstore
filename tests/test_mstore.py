...
def fill_storage(store: MStore, count: int, *, start: int = 0) -> MStore:
...
def read_storage(store: MStore, count: int, *, start: int = 0) -> MStore:
...
def test_list(mstore_mirror):
    mstore = fill_storage(mstore_mirror, 1024)
...
def test_list(mstore_jbod):
    mstore = fill_storage(mstore_jbod, 1024)
    # check if we get all expected keys from the jbod mstore
    assert list_store_names(mstore, "") == sorted([lkey(i) for i in range(1024)])


def test_load_store_list_distribution(mstore_jbod):
    mstore = fill_storage(mstore_jbod, 1024)
...
def test_load_store_list_redundancy(mstore_mirror):
    mstore = fill_storage(mstore_mirror, 1024)
...
def test_move_delete_undelete(mstore_mirror):
    ...
    assert list_store_names(store, "", deleted=False) == [k1]
    assert list_store_names(store, "", deleted=True) == [k0, k1]
    # undelete
    store.move(k0, undelete=True)  # undelete previously soft deleted item
    assert list_store_names(store, "", deleted=False) == [k0, k1]
    assert list_store_names(store, "", deleted=True) == [k0, k1]


def test_namespaces(mstore_jbod):
    store = mstore_jbod
    store.kinds = [("config/", "generic"), ("data/", "hex-hash")]
    store.store("config/main", b"some config")
    store.store("data/0000", b"value_00")
    store.store("data/bf00", b"value_bf")
    store.store("data/c000", b"value_c0")
    store.store("data/ff00", b"value_ff")
    # now let's check where stuff ended up being stored.
    st0, st1 = store.stores
